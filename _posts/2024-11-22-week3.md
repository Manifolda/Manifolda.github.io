...
## 复数与复变函数
### 复数域与扩充复平面
复数 $z=x+iy$，引入坐标，得到复平面 $\mathbb{C}$，引入无穷远点 $\infty$，组成扩充复平面 $\mathbb{C}^*=\mathbb{C} \cup \left\{\infty\right\}$
...
> 定义（导数）
> 
> $设f(z)是复平面上定义在区域D上的函数，z是复平面中的一个点，若对于所有的复数h，极限\lim_{h \to 0} \frac{f(z+h)-f(z)}{h}存在且相同，则称f(z)在z点可微，记作
f'(z)，\\pdx如果f在其定义域上每一点都可微，则称其为其定义域上的\textbf{解析函数}或\textbf{全纯函数}$
> 
{: .prompt-info }
...
> 定义（复积分）
> 
> $特别地，当\gamma分段可微时，其方程为z=z(t)(a\leq t\leq b),f(z)在\gamma上定义且连续，则f(z(t))也是t的连续函数$
